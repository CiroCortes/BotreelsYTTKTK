...
class HistoriaService:
    """
    Servicio para la generación y validación de historias y prompts.
    Cada método es autocontenible y maneja sus propios errores.
    """
    def procesar_historias_pendientes(self, historias: list[str]):
        """Procesa una lista de títulos de historias pendientes."""
        for titulo in historias:
            print(f"\n[NODO HISTORIA] Procesando: {titulo}")
            if self.validar_historia(titulo):
                print("   ✅ Historia y prompts ya existen y son válidos.")
                continue
            try:
                historia = self.generar_historia(titulo)
                if historia and self.validar_historia(titulo):
                    print("   ✅ Historia y prompts generados correctamente.")
                else:
                    print("   ❌ Error: No se pudo validar la historia generada para:", titulo)
            except Exception as e:
                print(f"   ❌ Error generando historia para '{titulo}': {e}")

    def generar_historia(self, titulo: str) -> Historia:
        """Genera la historia y los prompts para un título dado y retorna un modelo Historia."""
        ...
        # Música
        try:
            musica_dir = Path("musica/biblic_music")
            archivos_musica = list(musica_dir.glob("**/*.mp3"))
            if archivos_musica:
                musica_origen = random.choice(archivos_musica)
        ...
        return Historia(titulo, parrafos, prompts)

    def validar_historia(self, titulo: str) -> bool:
        """Valida si la historia y los prompts existen y son correctos."""
        titulo_limpio = sanitizar_titulo_para_directorio(titulo)
        historia_dir = Path("historias") / titulo_limpio
        prompts_path = historia_dir / "prompts.txt"
        parrafos_path = historia_dir / "parrafos.txt"
        if not prompts_path.exists() or not parrafos_path.exists():
            return False
        if prompts_path.stat().st_size == 0 or parrafos_path.stat().st_size == 0:
            return False
        ...
        if len(prompts) < 14 or len(parrafos) < 14:
            return False
        return True

    def cargar_historia(self, titulo: str) -> Historia:
        """Carga una historia existente desde archivos y retorna un modelo Historia."""
        titulo_limpio = sanitizar_titulo_para_directorio(titulo)
        historia_dir = Path("historias") / titulo_limpio
        parrafos_path = historia_dir / "parrafos.txt"
        prompts_path = historia_dir / "prompts.txt"
        if not parrafos_path.exists() or not prompts_path.exists():
            raise FileNotFoundError("No se encontraron los archivos de la historia.")
        return Historia.from_files(titulo, str(parrafos_path), str(prompts_path)) 